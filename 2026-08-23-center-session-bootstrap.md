...
4. ~~**【P1·구조결함】 골든 하네스가 negation 결함을 못 잡음**~~ → **오판. 취소한다(2026-08-23 01:5x 정정).**
   `tests/golden_regression.py:385 persona_negation_checks()`가 이미 그 사각지대를 막고 있다. `fired`가 아니라 **축 점수 자체가 0인지**를 직접 단언하고(`r["scores"][axis]==0 and r["total"]==0`), 추가로 적색등 2개(실위반은 여전히 가산 / `negation_patterns` 과확대 방지 8케이스)를 건다.
   → 앞선 판 §4의 "80/80은 negation 정확성의 증거가 아니다"는 **틀렸다.** 근거는 `2026-08-22-vending-to-coordinator.md` §1-3(결함 보고)만 읽고 그 뒤 커밋 `6f05bad`에서 보완된 것을 확인하지 않은 것
