...
| 2026-08-18 | ✅ **지시서 #55 완료** (trace phase1-harness-baseline-20260818) — 백테스트 하네스 + devig 기준선(공식 적합 없음, 순수 stdlib·numpy/pandas 미사용). [스크립트] `ops/backtest-harness-baseline.py`. [데이터] (배당+결과) 겹침 11011건 중 머니라인 없는 39건 제외 → **실험셋 10972**. [조인검증] game_pk 조인(더블헤더는 #54 load 시 start_time 구분), 팀명·날짜 교차검증 **불일치 0건**. [분할] train 6904(2021-23)/val 2381(2024)/holdout 1687(2025~8/16). [기준선] devig log-loss val **0.66762**/hold **0.67620**, Brier val **0.23767**/hold **0.24191**. [캘리브레이션] 십분위 예측↔실제 대체로 정합(양끝 과소·과대 편향 경미). [누수특징] game_date< 엄격, 특징 4개(home/away winpct_last10·rundiff_last10), 시즌집계·<= 금지. [자기검증] 스코어누수(home_score-away_score) 주입 → log-loss **0.00461/0.00495**(기준선 대비 ~145배 붕괴) → 제거 확인. [npm test] 14스위트 286건(JS 변경 0). [막힘] 없음 | ✅ |
[2026-08-21] 지시서 #57 — git write-path 검증: main 최신(dca6859) pull 확인, 할매봇 커밋+push 라운드트립 정상. (trace: git-verify-20260821)
[2026-08-21] 지시서 #60 완료 — sns-jping repo 접근권한 확인(read/clone/write 전부 가능, HEAD 2589e35 일치). #58 취소(#59): repo는 사용자 생성 + 센터장 push 완료. (trace: sns-jping-access-check-20260821)
