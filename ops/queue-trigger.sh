...
# 여기까지 왔다는 기록을 남긴다.
# 아래 exec 는 프로세스를 갈아치우므로 그 뒤로는 이 스크립트의 코드가 돌지 않는다.
# "cron 이 아예 안 떴는가 / 떴는데 큐가 비었는가" 를 나중에 구분하려면
# exec 앞에서 한 줄 남겨두는 수밖에 없다. (실제로 이 구분이 안 돼 헤맸다)
LOG="$ROOT/ops/queue-trigger.log"
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] trigger 시작 ROOT=$ROOT" >> "$LOG" 2>/dev/null || true

exec node "$ROOT/ops/queue-trigger.js" >> "$LOG" 2>&1
